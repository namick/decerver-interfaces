...
//var monklog = monklog.NewLogger("CLI")
...
/*
func NewThelonious(db monkutil.Database, clientIdentity monkwire.ClientIdentity, keyManager *crypto.KeyManager, usePnp bool, OutboundPort string, MaxPeer int) *eth.Thelonious {
	ethereum, err := eth.New(db, clientIdentity, keyManager, eth.CapDefault, usePnp)
	if err != nil {
		monklog.Fatalln("eth start err:", err)
	}
...
func StartRpc(ethereum *eth.Thelonious, RpcPort int) {
	...
	/*
		...
		if err != nil {
			monklog.Errorf("Could not start RPC interface (port %v): %v", RpcPort, err)
		} else {
			go ethereum.RpcServer.Start()
		}*/
}

var myMiner *monkminer.Miner
...
func StartMining(ethereum *eth.Thelonious) bool {
	...
	if !ethereum.Mining {
		...
		go func() {
			...
			if myMiner == nil {
				myMiner = monkminer.NewDefaultMiner(addr, ethereum)
			}
...
/*
func CheckZeroBalance(pipe *xeth.Xeth, keyMang *monkcrypto.KeyManager) {
	keys := keyMang.KeyRing()
	masterPair := keys.GetKeyPair(0)
	monklog.Infoln("master has ", pipe.Balance(keys.GetKeyPair(keys.Len()-1).Address()))
	for i := 0; i < keys.Len(); i++ {
		k := keys.GetKeyPair(i).Address()
		val := pipe.Balance(k)
		monklog.Infoln("key ", i, " ", monkutil.Bytes2Hex(k), " ", val)
		v := val.Int()
		if v < 100 {
			...
			if err != nil {
				monklog.Infoln("Error transfering funds to ", monkutil.Bytes2Hex(k))
			}
		}
	}
}*/
